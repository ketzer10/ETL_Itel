...
def sheet_downloader_and_uploader(file_id: str, sheet_name: str, read_with_dtype: dict, expected_columns: list, transform_function, info_transform_function: str, schema: str, table_name: str):
    ...
    df = pd.concat(pd.read_excel(file_obj["contents"], sheet_name=sheet_name, dtype = read_with_dtype)).reset_index(level=0,) # Read into a DataFrame
    print(df.head(20))
...
def main(optional: list):
    ...
    """
    FOLDER_KEY = "tds_hrm_employee_id"
    ...
    sheet_name = None
    schema = "tds"
    table_name = "hrm_employee_id"
    expected_columns = ["level_0","HRM ID","HRM Name","TDS Name","TDS ID","Site"]
    file_id = utils.get_config("dshub_sharepoint_config")["files"][FOLDER_KEY]["id"]
