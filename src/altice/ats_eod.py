...
def sheet_downloader_and_uploader(expected_columns: list, folder_key: str, schema: str, table_name: str):
    ...
    for file_id in file_ids:
        ...
        try:
            ...
            df = df[['date', 'half_hour_interval', 'agent_name', 'ps_id', 'organization', 'location', 'department', 'skill', 'language', 'occupancy_pct', 'active_sec', 'busy_sec', 'ready_sec', 'not_ready_sec', 'ats']]
...
def main(optional: list):
    ...
    expected_columns = ['organization','location','department','skill','language','agent_name','ps_id','date',
                        'half_hour_interval','occupancy_pct','active_sec','busy_sec','ready_sec','not_ready_sec','ats']

    match(optional[0]):
        case 1:
            folder_key = 'altice_mobile_ats_eod'
            table_name = 'glb_mobile_ats'
        case 2:
            folder_key = 'altice_optimum_ats_eod'
            table_name = 'glb_optimum_ats'
        case 3:
            folder_key = 'altice_suddenlink_ats_eod'
            table_name = 'glb_suddenlink_ats'        
