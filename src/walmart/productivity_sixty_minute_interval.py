...
def transformations(df, expected_columns):    
    ...
    intervals = df['sixty_minute_interval_start'].str.split('-', 2 ,expand=True)
    df['agent_name'] = df['agent_name'].str.split(',').str[::-1].str.join(' ').str.strip()
    
    df['sixty_minute_interval_start'] = intervals[0]
    date_interval = df['date'].astype('str')+' '+df['sixty_minute_interval_start']+':00'    
    df['sixty_minute_interval_start'] = pd.to_datetime(date_interval, format = "%Y-%m-%d %H:%M:%S")
...
def sheet_downloader_and_uploader(expected_columns: list, folder_key: str, schema: str, table_name: str):
    ...
    for file_id in file_ids:
        ...
        try:
            ...
            # Upload to database
            dbutils.perform_safe_delete_insert_with_keys(conn, ['sixty_minute_interval_start', 'team_name'], df, schema, table_name)
...
def main(optional: list):
    ...
    expected_columns = ['date','sixty_minute_interval_start','agent_name','agent_id','team_name','login_sec','holds',
                        'handled','number_of_logins','abandons','refused','answered','inbound','outbound','busy','pct_refused',
                        'acd_sec','outbound_handled','agent_abandons','short_abandons','agent_sec','talk_sec','wait_sec','available_sec',
                        'hold_sec','contact_sec','speed_of_answer','handle_sec','outbound_sec','inbound_sec','consult_sec','abandon_sec', 
                        'acw_sec', 'held', 'inbound_handled']     
    folder_key = 'walmart_prod'
    schema = 'walmart'
    table_name = 'glb_agent_productivity_sixty_min_interval'
