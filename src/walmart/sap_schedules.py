""" 
Downloads an Excel file containing Schedules from the SharePoint, uploads to the database and moves to archived.
"""
...
def transformations(df, expected_columns):    
    try:
        df = df[["Date", "Emp. ID", "Name", "Schedule_code", "Start", "End", "Break 1 Start", "Break 1 Stop", "Lunch Start", "Lunch Stop", "Break 2 Start", "Break 2 Stop", "schedule_duration", "first_break_duration", "second_break_duration", "lunch_duration"]]
    except Exception as e:
        print("Expected columns from schedule file not found.\nError Details:", e)
    ...
    try:
        ...
        print("Unable to rename columns.\nError details:",e,"\nColumns in df:", df.columns, "\nExpected Columns:", expected_columns)

    df.dropna(axis=0, subset=['legacy_id'],  inplace=True)     
    
    date_columns = ["schedule_start", "schedule_end", "first_break_start", "first_break_stop", "lunch_start", "lunch_stop", 
                    "second_break_start", "second_break_stop"]
    duration_columns = ["schedule_duration_sec", "first_break_duration_sec", 
                        "second_break_duration_sec", "lunch_duration_sec"]

    df["date"] = df["date"].astype("datetime64[ns]")

    df = dfutils.convert_to_datetime_columns(df, date_columns)    
    df["schedule"] = df["schedule_start"].dt.strftime("%H:%M") + " - " + df["schedule_end"].dt.strftime("%H:%M")

    df["legacy_id"] = df["legacy_id"].astype('str')
    df["legacy_id"] = df["legacy_id"].apply(lambda txt: txt.split(".")[0])  

    df = dfutils.validate_duration_columns(df, duration_columns)  
    df = dfutils.convert_duration_columns_to_sec(df, duration_columns)
...
def sheet_downloader_and_uploader(expected_columns: list, folder_key: str):
    ...
    for file_id in file_ids:
        ...
        try:
            ...
            df = pd.read_excel(file_obj["contents"], sheet_name="Lunch and Breaks ", skiprows = 1) # Read into a DataFrame
            ...
            # Upload to database
            dbutils.perform_safe_delete_insert_with_keys(conn = conn, delete_keys=["date"], source_df = df, 
                                                         schema = schema, target_table_name = table_name)
...
def main(optional: list):
    ...
    expected_columns = ["date", "legacy_id", "agent_name", "schedule_code", "schedule_start", 
                        "schedule_end", "first_break_start", "first_break_stop", "lunch_start", "lunch_stop", 
                        "second_break_start", "second_break_stop", "schedule_duration_sec", "first_break_duration_sec", 
                        "second_break_duration_sec", "lunch_duration_sec", "schedule"]
     
    folder_key = "walmart_schedules"
