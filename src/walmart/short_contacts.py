...
def transformations(df, expected_columns):    
    ...
    df["contact_start_date_time"] = pd.to_datetime(df["contact_start_date_time"], format = "%Y-%m-%d %H:%M:%S")
    df["contact_end_date_time"] = pd.to_datetime(df["contact_end_date_time"], format = "%Y-%m-%d %H:%M:%S")

    df["contact_duration_sec"] = (df["contact_end_date_time"] - df["contact_start_date_time"]).dt.seconds
...
def main(optional: list):
    ...
    expected_columns = ["date", "thirty_minute_interval_start", "agent_name", "agent_id", "skill_name", "team_name", 
                        "contact_end_reason", "contact_id", "master_contact_id", "contact_start_date_time", "contact_end_date_time", 
                        "handled", "contact_duration_sec"]
     
    folder_key = "walmart_short_contacts"
